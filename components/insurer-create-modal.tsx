...
interface InsurerCreateModalProps {
  open: boolean;
  onClose: () => void;
  onCreateInsurer: (data: { name: string }) => Promise<Insurer>;
  onCreated: (insurer: Insurer) => void;
}

export function InsurerCreateModal({
  open,
  onClose,
  onCreateInsurer,
  onCreated,
  ...
  const handleCreate = useCallback(async () => {
    ...
    setIsSaving(true);
    try {
      const created = await onCreateInsurer({ name: name.trim() });
      onCreated(created);
      handleClose();
    } catch (error) {
  ...
  }, [name, onCreateInsurer, onCreated, handleClose]);
            ...
            <Card className="relative overflow-hidden">
              <CardHeader className="pb-3 sm:pb-4">
                <CardTitle className="text-base sm:text-lg flex items-center gap-2 sm:gap-2.5">
                  ...
                  <span className="text-sm sm:text-base lg:text-lg">Informações da Asseguradora</span>
                </CardTitle>
              </CardHeader>
